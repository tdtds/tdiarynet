...
#
add_footer_proc do
	footer = '<div class="footer">This Diary is Running on <a href="https://www.tdiary.net/">tDiary.Net</a> 1st server. Hosted by <a href="https://www.interlink.or.jp/service/">インターリンク</a>&nbsp;<a href="https://hosting.interlink.or.jp/">ホスティング</a></div>'

	if @conf['tdiarynet_suspended']
		footer << %Q|<div style="position:absolute;top:0px;left:0px;color:#000;background-color:#fff;padding:32px;">この日記は更新が止まっているので変更不可になっています。再度更新したい場合には<a href="https://www.tdiary.net/bbs/support.html">掲示板<a>にてその旨をお知らせ下さい。折返し管理者より連絡します。</div>|
	end
...
def clear_tdiarynet_cache(date)
	begin
		url = URI("https://proxy.tdiary.net/cache/#{@conf.user_name}")
		puts "I, request to clear URL: #{url}"
		params = {proxy_key: ENV['PROXY_KEY']}
		if date
